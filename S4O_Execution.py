...
def S4O_Execution():
	...
	if 'df_Execution' not in st.session_state:
		...
		seaDur = float(st.session_state.df_Execution.iloc[0,1])
		wlRamp = float(st.session_state.df_Execution.iloc[1,1])
		maxRel = st.session_state.currentMaxRel
		sdTol  = float(st.session_state.df_Execution.iloc[3,1])
		odFac  = float(st.session_state.df_Execution.iloc[4,1])

	#	Prompt for execution parameters
	seaDur = st.number_input("Sea state duration [h] :", min_value=1.0, value=seaDur, format="%.2f")
	wlRamp = st.number_input("Wave load ramping time [s] :", min_value=1.0, value=wlRamp, format="%.1f")
	maxRel = st.number_input("Maximum number of sea state realisations [-] :", min_value=1, value=maxRel, step=1, format="%i")
	...
	odFac  = st.number_input("Design curve value as factor of outer diameter [-] :", min_value=1.0, value=odFac, format="%.1f")

	if st.session_state.modelFilePath == '':
		st.session_state.currentMaxRel = maxRel
	elif maxRel != int(st.session_state.df_Execution.iloc[2,1]) and os.path.exists(st.session_state.modelFilePath):
		...
		st.session_state.currentMaxRel = maxRel
		maxRel = int(st.session_state.df_Execution.iloc[2,1])
	else:
		st.session_state.currentMaxRel = maxRel

	# Update df_Execution dataframe
	Exdata = [['Sea state duration [h] :', seaDur], ['Wave load ramping time [s] :', wlRamp],
